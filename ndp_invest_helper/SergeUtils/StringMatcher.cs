...
namespace SergeUtils
{
    ...
    public class StringMatcher
    {
        ...
        public StringMatcher(StringMatchingMethod method, string pattern)
        {
            switch (method)
            {
                ...
                case StringMatchingMethod.UseRegexs:
                    ...
                    Match = buildRegex;
                    break;
                case StringMatchingMethod.SubString:
                    m_pattern = prepareNoWildcard(pattern);
                    Match = buildSubstring;
                    break;
                default:
        ...
        private StringMatch buildSubstring(string source)
        {
            string pattern = (string)m_pattern;
            string lsource = source.ToLower();

            return lsource.Contains(pattern) ?
                new StringMatch
                {
                    Text = source,
                    Segments = new List<string>() { source },
                    StartsOnMatch = true,
                }
                : null;
        }
